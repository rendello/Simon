...
class Match:
    ...
    async def get_buttons(self, potential_buttons):
        ...
        if buttons == "":
            await self.send_message(
                text="No emojis found! Defaulting to normal buttons", section="warning"
            )
            return "🔴💚🔷🍊"
        ...
        while True:
            ...
            if self.status == "failed":
                ...
                await self.send_message(
                    text=text,
                    section="thanks",
                    file=discord.File("Artwork/simón_end.png"),
                )
                break
    ...
    async def perform_turn(self):
        async def wait_and_check_button_press(self):
            ...
            sequence_check = await check_against_sequence(
                self.sequence, self.user_sequence
            )
    ...
    async def game_over(self):
        await self.send_message(
            text=f"Oh No!!! You lost on turn `{self.turn_no}`!", section="loss"
        )
        await asyncio.sleep(0.5)
        await self.append_to_message(
    ...
    async def intro_sequence(self):
        await self.send_message(
            text="Welcome to *Simón!*",
            file=discord.File("Artwork/simón_instructions.png"),
            section="intro",
        )
...
async def simon(ctx, *, potential_buttons="🔴💚🔷🍊"):
    ...
    await matches[ctx.author.id].async_init(
        ctx=ctx, potential_buttons=potential_buttons
    )
